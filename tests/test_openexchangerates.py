...
RATES = {"EUR": Decimal(2), "GBP": Decimal(4), "BTC": Decimal(10)}
...
def conversion_rates(db):
    ...
    for currency, value in RATES.items():
        rate = ConversionRate.objects.create(to_currency=currency, rate=RATES[currency])
        rates.append(rate)
    return rates
...
def test_conversionrate__str_repr(conversion_rates):
    ...
    assert f"pk={obj.pk}," in obj_repr
...
def test_exchange_currency_uses_passed_conversion_rate():
    ...
    converted_value = exchange_currency(value, "GBP", conversion_rate=custom_rate)
...
def test_exchange_currency_for_money_range_uses_passed_conversion_rate():
    ...
    value_converted = exchange_currency(value, "GBP", conversion_rate=custom_rate)
    assert value_converted.currency == "GBP"
...
def test_exchange_currency_raises_for_nonsupported_type():
    with pytest.raises(TypeError):
        ...
        exchange_currency(PseudoMoneyType(), "GBP")
    with pytest.raises(AttributeError):
        exchange_currency("str", "GBP")
...
def test_get_rates_force_update_cache(conversion_rates):
    expected_cache_content = {rate.to_currency: rate for rate in conversion_rates}
